...
class Rectangle:
    ...
    def contains(self, point: Point2D, tolerance: float = 0) -> bool:
        ...
        return self._is_value_in_interval(ll_px, 0, self._dx, tolerance) \
            and self._is_value_in_interval(ll_py, 0, self._dy, tolerance)
    
    def _is_value_in_interval(self, i: int, min: int, max: int, tolerance: float) -> bool:
        return i >= min - tolerance and i <= max + tolerance
